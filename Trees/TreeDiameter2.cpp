...
int dis[MX][2];
//segtree<max_t>mt; // to fetch the min for toggling
 
int dfs(int cur,int pr,int disi,int i)
{
    dis[cur][i]=disi;
//  cout<<dis[cur][i]<<"yo ";
    int as=-1;
    for(auto k:adj[cur])
    {
        if(k!=pr)
        {
            ...
            int ch = dfs(k,cur,disi+1,i);
            if(as==-1 || dis[ch][i]>dis[as][i]){as=ch;}
...
int f_far= dfs(0,0,0,0);
//cout<<f_far<<" ";
//cout<<endl;
int s_far= dfs(f_far,f_far,0,0);
//cout<<s_far<<endl;
dfs(s_far,s_far,0,1);
